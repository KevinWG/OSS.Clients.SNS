    ...
    internal static class WechatChatHelper
    {
            ...
            string timestamp, string nonce,string strEncryptMsg)
        {
            return msgSignature == GenerateSignature(token, timestamp, nonce, strEncryptMsg)
                ? new Resp() 
                : new Resp(RespCodes.ParaSignError, "微信签名验证失败！");
        }
        ...
        internal static StrResp EncryptMsg(string sReplyMsg, WechatMsgConfig config)
        {
            ...
            {
                return new StrResp().WithResp(SysRespCodes.AppError, "加密响应消息体出错！");
            }
            ...
            {
                return new StrResp().WithResp(SysRespCodes.AppError, "生成签名信息出错！");
            }
