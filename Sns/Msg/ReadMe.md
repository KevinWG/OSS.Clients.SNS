...
一. 配置管理
	...
	示例详见：OSS.SnsSdk.Samples项目下的WxMsgController.cs
...
二. 框架使用
	...
	2. Handler，消息处理控制类，实现整个消息处理的生命周期和执行调度
		当前系统有 WxMsgBaseHandler 和 WxMsgHandler 两个，前者作为基类，实现了生命周期的控制和调度。
	...
	1. 基础模式 
	系统 WxMsgHandler.cs 默认实现常见的六种普通消息和五种事件消息，只需要重写（overwrite）对应的以 Process 开头的方法即可。
...
三. 生命周期扩展
	...
	在系统处理的不同阶段，我定义了几个主要的处理事件，来满足对消息处理时的全局和局部控制,分别对应在WxMsgBaseHandler的以Execute开头的虚方法：

	1. Executing(WxMsgContext context)，开始执行事件，作为范围为全部消息类型。
	...
	2. ExecuteUnknowProcessor(WxBaseRecMsg msg), 未知消息类型事件，作用范围为所有未发现对应处理委托的消息类型。
		...
		需要注意的是，即使你使用的是 WxMsgHandler ，如果没有重写其 实现，或者返回了为空， 也会触发此方法

		可以通过这个方法中添加未知类型消息日志等

	3. ExecuteEnd(WxMsgContext msgContext), 执行结束时调用的方法，作为范围为全部消息类型。
...
四. 常见问题 
	...
	如果你的控制类直接继承了 WxMsgBaseHandler 则 不会进入基础模式
...
五. 终极大招
	...
	如果你希望自己定义一套完整的生命周期，ok，重写这里即可。只需要记住一件事情，如果你重写了这里，上述的几种模式和其他生命周期事件将无效。
